...
void SMTLIBInterpreter::addVarBinding(const std::string &name,
                                      const z3::expr &expr)
{
    variableBindings.push_back({name, expr});
}

void SMTLIBInterpreter::addFunctionDefinition(const std::string &name,
...
void SMTLIBInterpreter::addSortDefinition(const std::string &name,
                                          const z3::sort &sort)
{
...
{
    auto varItem =
        std::find_if(variables.rbegin(), variables.rend(),
    ...
    auto bindItem =
        std::find_if(variableBindings.rbegin(), variableBindings.rend(),
                     [name](const auto &it) { return it.first == name; });
...
antlrcpp::Any
SMTLIBInterpreter::visitCommand(SMTLIBv2Parser::CommandContext *command)
{
    if (exited) {
        return antlrcpp::Any{};
    }
    ...
    if (command->cmd_setLogic()) {
        ...
        if (info->PK_Authors()) {
            std::cout << "(:authors \"Martin Jonas, Jan Strejcek\")"
                      << std::endl;
        } else if (info->PK_AssertionStackLevels()) {
            std::cout << "(:assertion-stack-levels " << (asserts.size() - 1)
                      << ")" << std::endl;
        ...
        if (option->PK_DiagnosticOutputChannel()) {
            ...
            std::cout << ":print-success " << (printSuccess ? "true" : "false")
                      << std::endl;
        } else if (option->PK_ProduceModels()) {
            std::cout << ":produce-models "
                      << (config.produceModels ? "true" : "false") << std::endl;
        } else if (option->PK_RegularOutputChannel()) {
            // TODO
        } else if (option->PK_Verbosity()) {
        ...
        if (sorts.size() != 1) {
            std::cout << "Uninterpreted functions are not supported"
                      << std::endl;
            exit(1);
        }
...
antlrcpp::Any
SMTLIBInterpreter::visitSorted_var(SMTLIBv2Parser::Sorted_varContext *sv)
{
    return addVar(sv->symbol()->getText(),
                  std::any_cast<z3::sort>(visitSort(sv->sort())));
}

antlrcpp::Any
SMTLIBInterpreter::visitVar_binding(SMTLIBv2Parser::Var_bindingContext *sv)
{
    addVarBinding(sv->symbol()->getText(),
                  std::any_cast<z3::expr>(visitTerm(sv->term())));
    return antlrcpp::Any{};
}
...
antlrcpp::Any
SMTLIBInterpreter::visitHexadecimal(SMTLIBv2Parser::HexadecimalContext *b)
{
...
antlrcpp::Any
SMTLIBInterpreter::visitFunction_def(SMTLIBv2Parser::Function_defContext *fd)
{
    ...
    addFunctionDefinition(name, args,
                          std::any_cast<z3::expr>(visitTerm(fd->term())));

    variables.clear();
    return antlrcpp::Any{};
}

z3::expr SMTLIBInterpreter::applyDefinedFunction(const std::string &name,
                                                 const z3::expr_vector &args)
{
...
antlrcpp::Any SMTLIBInterpreter::visitTerm(SMTLIBv2Parser::TermContext *term)
{
    ...
    if (term->GRW_Forall()) {
        ...
        z3::expr result = z3::forall(
            bound, std::any_cast<z3::expr>(visitTerm(term->term(0))));
    ...
    if (auto ident = term->qual_identifer()->identifier()) {
        ...
        if (identName == "not") {
            ...
            if (subterms[0].get_sort().is_bool()) {
                if (subterms.size() != 2) {
                    std::cout << "Unsupported Boolean distinct of arity > 2"
                              << std::endl;
                    exit(1);
                }
                return !(subterms[0] == subterms[1]);
