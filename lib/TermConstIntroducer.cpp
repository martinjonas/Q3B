...
std::pair<z3::expr, std::set<OpVar>>
TermConstIntroducer::flattenMulRec(const z3::expr &e,
                                   const std::vector<Var> &boundVars)
{
    auto item = flattenMulCache.find(e);
    if (item != flattenMulCache.end() &&
        boundVars == std::get<1>(item->second)) {
    ...
    if (e.is_var()) {
        ...
        if ((decl_kind == Z3_OP_BMUL || decl_kind == Z3_OP_BSDIV ||
             decl_kind == Z3_OP_BSDIV_I) &&
            ...
            if (isVar(lExpr) && isVar(rExpr)) {
                ...
                if (lExpr.is_var()) {
                    ...
                    int deBruijnIndex = Z3_get_index_value(*context, ast);
                    lExpr =
                        boundVars[boundVars.size() - deBruijnIndex - 1].expr;
                }
                ...
                } else if (decl_kind == Z3_OP_BSDIV ||
                           decl_kind == Z3_OP_BSDIV_I) {
                    newName << "bvsdiv_" << lExpr << "_" << rExpr;
                }

                std::set<OpVar> mulVars;
                auto newMulExpr = context->bv_const(newName.str().c_str(),
                                                    e.get_sort().bv_size());
        ...
        BoundType boundType =
            Z3_is_quantifier_forall(*context, ast) ? UNIVERSAL : EXISTENTIAL;
        ...
        for (int i = 0; i < numBound; i++) {
            ...
            if (s.is_bv()) {
                auto v = context->bv_const(current_symbol.str().c_str(),
                                           s.bv_size());
                std::copy_if(
                    mulVars.begin(), mulVars.end(),
                ...
                std::copy_if(mulVars.begin(), mulVars.end(),
        ...
        for (const auto &mulVar : LquantifiedOpVars) {
            if (mulVar.op == Z3_OP_BMUL) {
                ...
                for (const auto &v1 : varsLInMul[mulVar.r.to_string()]) {
                    ...
                    auto equivOp =
                        context->bv_const(newName.str().c_str(),
...
void TermConstIntroducer::fillVarsInMul(const z3::expr &e)
{
    ...
    if (e.is_app()) {
        ...
        // TODO: also BVMUL of arity > 2
        if (decl_kind == Z3_OP_BMUL && numArgs == 2 && isVar(e.arg(0)) &&
            isVar(e.arg(1))) {
            auto l = e.arg(0);
            auto r = e.arg(1);
