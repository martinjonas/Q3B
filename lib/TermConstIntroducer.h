...
{
public:
    TermConstIntroducer(z3::context &ctx) { this->context = &ctx; }
...
private:
    ...
    struct Var {
        ...
        bool operator==(const Var &rhs) const { return name == rhs.name; }
    };

    z3::context *context;
    std::pair<z3::expr, std::set<OpVar>>
    flattenMulRec(const z3::expr &, const std::vector<Var> &);
    ...
    std::map<const Z3_ast,
             std::tuple<z3::expr, const std::vector<Var>, std::set<OpVar>>>
        flattenMulCache;
};
